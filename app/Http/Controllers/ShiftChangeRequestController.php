...
class ShiftChangeRequestController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $rules = [
            ...
            'turno' => 'required|in:Mañana,Tarde,Noche,Día Completo,Mañana y tarde,Tarde y noche,Mañana y noche',
        ...
        if ($request->fecha2) {
            ...
            foreach ($guardsInBetween as $guard) {
                $brigadeEmp1 = $this->getBrigadeForDate($request->id_empleado1, $guard->date);
                $brigadeEmp2 = $this->getBrigadeForDate($request->id_empleado2, $guard->date);
     ...
     * Maneja correctamente: asignaciones mismo día (ida/vuelta),
     * brigadas excluidas, y prioridad de turno.
     */
    private function getBrigadeForDate($idEmpleado, $fecha)
    {
        ...
        // 1. Buscar asignaciones del mismo día
        ...
        // 2. Si no hay del mismo día, buscar la última asignación anterior

...
class IncidentsRepository {
    ...
    suspend fun createIncident(incident: CreateIncidentRequest): Result<Incident> {
        return try {
            ...
            Log.d(TAG, "Tipo: ${incident.tipo}, Nivel: ${incident.nivel}")
    ...
    suspend fun markAsRead(id: Int): Result<Incident> {
        return try {
    ...
    suspend fun resolveIncident(id: Int, resolution: ResolveIncidentRequest): Result<Incident> {
        return try {
            ...
            Log.d(TAG, "Resuelto por: ${resolution.resulta_por}")
    ...
    suspend fun countPending(): Result<CountResponseIncident> {
        return try {
            ...
            if (response.isSuccessful && response.body() != null) {
                Log.d(TAG, "✅ ${response.body()!!.pending} incidencias pendientes")
                Result.success(response.body()!!)
            } else {
                val errorMsg = "Error: ${response.code()}"
                Log.e(TAG, "❌ $errorMsg")

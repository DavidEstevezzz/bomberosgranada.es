...
const ShiftChangeRequestPage = () => {
  ...
  // ─── NUEVOS ESTADOS: guardias seleccionadas ───
  const [selectedGuardDate, setSelectedGuardDate] = useState(null);
  const [selectedGuardDate2, setSelectedGuardDate2] = useState(null);
  ...
  // ─── Forzar turno a Dia Completo en espejo ───
  useEffect(() => {
  ...
  // ─── Si no es jefe, establecer automáticamente el primer empleado ───
  useEffect(() => {
  ...
  // ─── Limpiar guardias seleccionadas cuando cambia tipo o empleado 1 ───
  useEffect(() => {
    setSelectedGuardDate(null);
    setSelectedGuardDate2(null);
  }, [changeType, selectedEmployeeId]);

  // ─── Limpiar guardia 2 cuando cambia empleado 2 ───
  useEffect(() => {
    setSelectedGuardDate2(null);
  }, [selectedEmployeeId2]);
  ...
  const handleSubmit = async (e) => {
    ...
    // ─── Validar guardias seleccionadas ───
    if (!selectedGuardDate) {
      setError(isJefe ? 'Selecciona la guardia del primer bombero.' : 'Selecciona tu guardia para el cambio.');
      return;
    }

    if (changeType === 'espejo' && !selectedGuardDate2) {
      setError(isJefe ? 'Selecciona la guardia del segundo bombero.' : 'Selecciona la guardia del compañero para el cambio espejo.');
      return;
    }
    ...
    try {
      const requestData = {
        ...
        fecha: selectedGuardDate,
        fecha2: changeType === 'espejo' ? selectedGuardDate2 : null,
      ...
      // Intentar mostrar mensaje del backend (validación espejo)
      const backendMsg = err?.response?.data?.message;
      setError(backendMsg || 'Error al enviar la solicitud de cambio de guardia.');
    }
  };
    ...
    <div className={cardContainerClass}>
      ...
      <div className="space-y-8 px-6 py-8 sm:px-10">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          {/* ─── SECCIÓN 2: Selección de bomberos ─── */}
          <section className={sectionBaseClass}>
          ...
          {/* ─── SECCIÓN 3: Selección de guardias ─── */}
          <section className={sectionBaseClass}>
            {/* Guardia del bombero 1 */}
            <div className="mb-2">
              <p className="text-sm font-semibold text-primary-600 dark:text-primary-200">
                {isJefe ? 'Guardia del primer bombero' : 'Tu guardia'}
              </p>
              <p className={`mt-1 mb-4 text-xs ${subtleTextClass}`}>
                {changeType === 'espejo'
                  ? 'Selecciona la guardia para el cambio espejo.'
                  : 'Selecciona la guardia que quieres cambiar.'}
              </p>
            </div>

            {selectedEmployeeId ? (
              <GuardSelector
                key={`guard1-${selectedEmployeeId}-${changeType}`}
                idEmpleado={selectedEmployeeId}
                darkMode={darkMode}
                onSelectionChange={(guards) => setSelectedGuardDate(guards.length > 0 ? guards[0] : null)}
                tipoSolicitud="cambio_guardia"
                multiple={false}
                infoText={changeType === 'espejo'
                  ? 'ℹ️ Selecciona tu guardia para el cambio espejo. Después selecciona la del compañero.'
                  : 'ℹ️ Selecciona la guardia que quieres cambiar. Después elige el turno.'}
                counterLabel="Seleccionada"
                showBalance={false}
              />
            ) : (
              <div className={`rounded-2xl border px-4 py-6 text-center text-sm ${
                darkMode ? 'border-slate-700 bg-slate-800/40 text-slate-400' : 'border-slate-200 bg-slate-50 text-slate-500'
              }`}>
                {isJefe ? 'Selecciona el primer bombero para ver sus guardias.' : 'Cargando tus guardias...'}
              </div>
            )}

            {/* Cambio simple: selector de turno */}
            {changeType === 'simple' && selectedGuardDate && (
            ...
            {/* Cambio espejo: selector de guardia del compañero */}
            {changeType === 'espejo' && selectedGuardDate && selectedEmployeeId2 && (
              <div className="mt-8">
                <div className="mb-2">
                  <p className="text-sm font-semibold text-primary-600 dark:text-primary-200">
                    {isJefe ? 'Guardia del segundo bombero' : 'Guardia del compañero'}
                  </p>
                  <p className={`mt-1 mb-4 text-xs ${subtleTextClass}`}>
                    Selecciona la guardia del compañero para completar el cambio espejo. Recuerda que no puede haber guardias de ninguno de los dos en medio.
                  </p>
                </div>

                <GuardSelector
                  key={`guard2-${selectedEmployeeId2}-${selectedGuardDate}`}
                  idEmpleado={selectedEmployeeId2}
                  darkMode={darkMode}
                  onSelectionChange={(guards) => setSelectedGuardDate2(guards.length > 0 ? guards[0] : null)}
                  tipoSolicitud="cambio_guardia"
                  multiple={false}
                  infoText="ℹ️ Selecciona la guardia del compañero. No debe haber guardias de ninguno de los dos entre ambas fechas."
                  counterLabel="Seleccionada"
                  showBalance={false}
                />
              </div>
            ...
            {/* Aviso espejo: necesitas seleccionar compañero */}
            {changeType === 'espejo' && selectedGuardDate && !selectedEmployeeId2 && (
              <div className={`mt-6 rounded-2xl border px-4 py-3 text-sm font-medium ${
                ...
                ℹ️ Selecciona un compañero arriba para ver sus guardias disponibles para el cambio espejo.
              </div>
            )}
          </section>

...
const GuardSelector = ({
    ...
    return (
        <div className="space-y-5">
            ...
            <div className={`grid gap-3 ${daysPerGuard ? 'grid-cols-3' : showBalance ? 'grid-cols-2' : 'grid-cols-1'}`}>
            ...
            {/* Info */}
            <div className={`rounded-2xl border px-4 py-3 text-xs ${darkMode ? 'border-blue-500/20 bg-blue-500/5 text-blue-300' : 'border-blue-200 bg-blue-50 text-blue-700'
                }`}>
    ...
    return (
        ...
        >
            ...
            <div className="flex items-center gap-3">
                {/* Fecha */}
                <div className="text-center">
                    <p className={`text-2xl font-black leading-none ${isSelected ? 'text-primary-500' : (darkMode ? 'text-slate-100' : 'text-slate-800')
                        }`}>
                        {dayFormatted}
                    </p>
                    <p className={`mt-0.5 text-[10px] font-bold uppercase tracking-wider ${darkMode ? 'text-slate-500' : 'text-slate-400'
                        }`}>
                        {dayName}
                    </p>
                </div>
                ...
                {!hasExistingRequest && (
                    <div className={`flex h-6 w-6 flex-shrink-0 items-center justify-center ${multiple ? 'rounded-lg' : 'rounded-full'} border-2 transition-all duration-200 ${isSelected
